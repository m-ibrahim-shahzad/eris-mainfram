...
def process_ai_prompt():
    ...
    try:
        ...
        result = route(prompt, history_buffer)
        ai_response = result.get("response", "No response returned.")
        model_badge = result.get("model_used", "UNKNOWN")
        ...
        conn.rollback()
        print(f"Upstream routing fault: {str(e)}")
        return jsonify({
            "error": f"Processing pipeline failed. Upstream routing error. Detail: {str(e)}"
        }), 500
